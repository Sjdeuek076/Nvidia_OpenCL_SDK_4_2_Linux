...
int main() {
   ...
   //-----------------------------
   size_t dim = 2;
   size_t global_offset[] = {3, 5}; // first global id is {3, 5}; NULL for default {1,1}
									 //figure 5.1 on p98, the for loop  
									 // for(i=3; i < 9 ; i++) {
									 //    for (j=5; i < 9; j++) {
									 //			process(data(i,j));
									 //}
									 //}	
   size_t global_size[] = {6, 4};//global size is {6,4}    //result size 4 x 6 (rows x coloums)
   size_t local_size[] = {3, 2};// configures four work-groups by setting a local size equal to {3,2
   float test[24]; // containers for storing device buffers to host   
				   //size = global_size [] = {6, 4} = 6 x 4 = 24	
   cl_mem test_buffer;
   //-----------------------------
